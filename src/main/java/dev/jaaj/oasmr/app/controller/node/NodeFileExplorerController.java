...
import dev.jaaj.oasmr.lib.ObjectCreatedHandler;
import dev.jaaj.oasmr.lib.filemanagement.*;
...
public class NodeFileExplorerController extends View {
    ...
    private ObservableList<TreeItem<FileAdapter>> buildChildren(TreeItem<FileAdapter> treeItem) throws ExecutionException, InterruptedException {
        ...
        if (list.length != 0) {
            ...
            Future<? extends Serializable>[] futures = new Future[list.length];
            int i = 0;
            int id = nodesModel.getCurrentNodeData().iterator().next().getId();
            for (String childFile : list) {
                futures[i] = requestManager.aSyncSendRequest(new RequestExecuteCommand(id,
                        new CommandIsDirectory(childFile)));
                i++;
            }
            i=0;
            for (String childFile : list) {
                Boolean isDirectoryReponse = (Boolean) futures[i].get();
